$(document).ready(function() {
    fireBaseLogEvent("app_started", "page_view", "home");
	initialize();
    $('#highScoreNameForm').on('submit', function () {
        var highScoreBeat = proveHighScores();
        if (highScoreBeat == 'day' || highScoreBeat == 'all_time') {
        	if (highScoreBeat == 'all_time') {
        		$('#highScoreType').html('all-time');
			} else {
                $('#highScoreType').html('today');
			}
            $('#highScoresSpinner').show();
            $.ajax({
                type: "POST",
                url: "http://ec2-13-58-63-51.us-east-2.compute.amazonaws.com/hexotris",
                data: $('#highScoreNameForm').serialize() + '&highScoreNumber=' + score,
                success: function (data) {
                    writeHighScoresTable(data['day'], data['all_time']);
                    $('#highScoresSpinner').hide();
                }
            });
            $('#highScoreNameFormContainer').hide();
		}
    });
    getHighScores();
    $('#socialShare').on('click', function () {
        window.open('https://twitter.com/intent/tweet?text=Can you beat my score of '+ score +' points at&button_hashtag=hexotris ? https://goo.gl/hLtR2j @hexotris');
    });
    $('.aff-banner .close-button').on('click', function () {
        fireBaseLogEvent("aff_banner_hide_ma_1", "page_view", "home");
		$(this).parent().hide();
    });
});

function fireBaseLogEvent(eventName, contentType, itemId) {
    try {window.FirebasePlugin.logEvent(eventName, {content_type: contentType, item_id: itemId});} catch (error) {}
}

var highscoresFromServer;
function getHighScores() {
    $.ajax({
        type: "GET",
        url: "http://ec2-13-58-63-51.us-east-2.compute.amazonaws.com/hexotris",
        success: function (data) {
            highscoresFromServer = data;
            writeHighScoresTable(data['day'], data['all_time']);
        }
    });
}

function proveHighScores() {
	var i;
    for (i = 0; i < 10; ++i) {
        if (typeof highscoresFromServer['all_time'][i] == 'undefined' || score > highscoresFromServer['all_time'][i]['number']) {
            return 'all_time'
        }
    }
    for (i = 0; i < 10; ++i) {
        if (typeof highscoresFromServer['day'][i] == 'undefined' || score > highscoresFromServer['day'][i]['number']) {
            return 'day'
        }
    }
}

function writeHighScoresTable(todayHighScores, allTimeHighScores) {
    var i;
    for (i = 0; i < 10; ++i) {
        if (typeof todayHighScores[i] != 'undefined') {
            $('#today' + (i+1) + 'PlaceNumber').html(todayHighScores[i]['number']);
            $('#today' + (i+1) + 'PlaceName').html(todayHighScores[i]['name']);
        } else {
            $('#today' + (i+1) + 'PlaceNumber').html('');
            $('#today' + (i+1) + 'PlaceName').html('');
		}
    }
    for (i = 0; i < 10; ++i) {
        if (typeof allTimeHighScores[i] != 'undefined') {
            $('#allTime' + (i+1) + 'PlaceNumber').html(allTimeHighScores[i]['number']);
            $('#allTime' + (i+1) + 'PlaceName').html(allTimeHighScores[i]['name']);
        } else {
            $('#allTime' + (i+1) + 'PlaceNumber').html('');
            $('#allTime' + (i+1) + 'PlaceName').html('');
        }
    }
}
